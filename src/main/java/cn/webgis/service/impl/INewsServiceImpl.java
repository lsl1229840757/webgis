package cn.webgis.service.impl;

import cn.webgis.pojo.Chart;
import cn.webgis.pojo.News;
import cn.webgis.pojo.WordCloud;
import cn.webgis.service.NewsService;
...
public class INewsServiceImpl implements NewsService {
    ...
    public String getAllForPoint() {
        List<News> newsList = sqlSessionFactory.openSession().selectList("cn.webgis.mapper.NewsMapper.getNews");
    ...
    public String getNewsForEcharts() {
        List<News> newsList = sqlSessionFactory.openSession().selectList("cn.webgis.mapper.NewsMapper.getTenNews");

        JSONArray jsonArray = new JSONArray();
        for (int i = 0; i < newsList.size(); i++) {
            News news = newsList.get(i);
            String url = sqlSessionFactory.openSession().selectOne("cn.webgis.mapper.NewsMapper.getNewsUrl", news.getNews_id());
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("name", news.getNews_id());
    ...
    public String getProvinceMap() {
        ...
        for(int i=0;i<provinceArr.length;i++) {
            List<News> newsList = session.selectList("cn.webgis.mapper.NewsMapper.getProvinceNews",provinceArr[i]);
            for (int j = 0; j < newsList.size(); j++) {
                News news = newsList.get(j);
                JSONObject jsonObject2 = new JSONObject();
                String url = session.selectOne("cn.webgis.mapper.NewsMapper.getNewsUrl", news.getNews_id());
    ...
    public String getHotNews() {
        List<WordCloud>  wordClouds = sqlSessionFactory.openSession().selectList("cn.webgis.mapper.NewsMapper.getHotNews");
    ...
    public String getTitle(String news_id) {
        String title = sqlSessionFactory.openSession().selectOne("cn.webgis.mapper.NewsMapper.getTitle",news_id);
        return title;
    }
    ...
    public String getColdNews() {
        List<News> newsList = sqlSessionFactory.openSession().selectList("cn.webgis.mapper.NewsMapper.getColdNews");
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("type", "FeatureCollection");
        JSONArray jsonArray = new JSONArray();
        for (int i = 0; i < newsList.size(); i++) {
    ...
    public String getWarmNews() {
        List<News> newsList = sqlSessionFactory.openSession().selectList("cn.webgis.mapper.NewsMapper.getWarmNews");
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("type", "FeatureCollection");
        JSONArray jsonArray = new JSONArray();
        for (int i = 0; i < newsList.size(); i++) {
    ...
    public void addData(News news) {
        ...
        session.update("cn.webgis.mapper.NewsMapper.addData", addMap);
    }

    @Override
    public void deleteData(String comment_id) {
        SqlSession session = sqlSessionFactory.openSession();
        session.delete("cn.webgis.mapper.NewsMapper.deleteData", Integer.parseInt(comment_id));
    }

    @Override
    public String queryChartData() {
        List<Chart> chartList = sqlSessionFactory.openSession().selectList("cn.webgis.mapper.NewsMapper.queryChartData");
        Gson gson = new Gson();
